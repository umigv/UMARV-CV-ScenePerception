...
def main():
    ...
    hsv_obj.tune("yellow",image_mode=True)
    hsv_obj.tune("white",image_mode=True)
    
    # To display some values.
    time.sleep(1)
    
    # To review your masks, combined, invidually, and the raw image feed.
    # Will also diplay on the bottom a terminal to showcase detection.
    hsv_obj.post_processing_all_mask(image_mode=True)
